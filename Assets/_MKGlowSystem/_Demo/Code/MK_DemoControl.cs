...
public class MK_DemoControl : MonoBehaviour 
{
	...
	private void ManageRoom1()
	{
		...
		room1Objects [currentRoom1Object].renderer.material.shader = room1Shaders [currentRoom1Shader];

		if(currentRoom1Shader == 2)
			room1Objects [currentRoom1Object].renderer.material.SetTexture ("_ToonShade", cm);
		else if(currentRoom1Shader == 1)
		{
			room1Objects [currentRoom1Object].renderer.material.SetFloat ("_RimPower", room1RimP);
			room1Objects [currentRoom1Object].renderer.material.SetColor ("_RimColor", colors[currentRimColor]);
		}

		room1Objects [currentRoom1Object].renderer.material.SetColor ("_MKGlowColor", colors[currentRoom1GlowColor]);
		room1Objects [currentRoom1Object].renderer.material.SetColor ("_MKGlowTexColor", colors[currentRoom1GlowTexColor]);
		room1Objects [currentRoom1Object].renderer.material.SetTexture ("_MKGlowTex", room2Tex [currentRoom1Texture]);
		room1Objects [currentRoom1Object].renderer.material.SetFloat ("_MKGlowPower",room1GlowIntensity);
		room1Objects [currentRoom1Object].renderer.material.SetFloat ("_MKGlowTexStrength", room1GlowTextureStrength);
		room1Objects [currentRoom1Object].renderer.material.SetFloat ("_MKGlowOffSet", room1GlowWidth);
	...
	private void ManageRoom2()
	{
		...
		if(GUI.Button(new Rect(1500, 950, 400, 50), "Switch Texture", bStyle))
		{
			...
			for(int i = 0; i < 10; i++)
			{
				room2Objects[i].renderer.material.SetTexture("_MKGlowTex",room2Tex[currentRoom2Texture]);
			}
		}
	}
}
